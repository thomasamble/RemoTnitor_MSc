...
Plotting alpha diversity metrics for rarefied data against non-rarefied data and adding a 1:1 reference line for comparability. Also testing correlation between alpha diversity metrics before and after rarefaction, using `cor.test()` with Pearson's correlation.
...
cor.test(alpha_comp$Observed_main, alpha_comp$Observed_rarefied)
```
...
#### CLR/Aitchisons comparison

Aggregating the non-rarefied phyloseq object to species level, and applying a centered log-ratio (CLR) transformation to account for the compositional nature of the data. Aitchison distances were then calculated as Euclidean distances on the CLR-transformed data using `microViz::dist_calc()`, and ordination was performed using principal component analysis (PCA) with `microViz::ord_calc()`. The PCA ordination was visualized using `microViz::ord_plot()`, with 95% confidence ellipses, and points colored by season, location, and animal ID.

```{r}
set.seed(123)
aitchison_pca <- main_ps %>% 
  tax_agg(rank = "species") %>% 
  tax_transform("clr") %>% 
  ord_calc("PCA")

a_season <- aitchison_pca %>% 
  ord_plot(colour = "season", size = 3) +
  stat_ellipse(aes(group = season, fill = season, color = season),
               type = "t", level = 0.95, geom = "polygon", alpha = 0.2) +
  scale_colour_paletteer_d("ltc::trio4") +
...
a_season + a_location + a_id +
  plot_layout(ncol = 1)

ggsave(
  "plots/aitchison_pca.png",
  width = 7,
  height = 10,
  dpi = 300
)
...
Performing permutational multivariate analysis of variance (PERMANOVA) on Aitchison distance matrices using `microViz::dist_permanova()` (a wrapper around `vegan::adonis2()`) at species level with 99999 permutations. As for the Bray–Curtis approach, models were first fitted with season as the explanatory variable. Model results were extracted using `microViz::perm_get()`.

Beta dispersion was assessed using `vegan::betadisper()` on the Aitchison distance matrix to evaluate homogeneity of group dispersions. Pairwise PERMANOVA was performed using `pairwiseAdonis::pairwise.adonis2()` for both season and location, allowing comparison with the Bray–Curtis-based results.

```{r}
set.seed(123)
res_aitchison <- main_ps %>%
  tax_agg(rank = "species") %>%
  tax_transform("clr") %>%
  dist_calc("euclidean") %>%
  dist_permanova(variables = "season",
...
perm_get(res_aitchison) # also significant effect of season


aitchison_mat <- main_ps %>%
  tax_agg(rank = "species") %>%
  tax_transform("clr") %>%
  dist_calc("euclidean") %>% 
  dist_get()

betadisp_aitchison <- betadisper(aitchison_mat, metadata %>% slice(1:25) %>% pull(season))
betadisp_aitchison %>% anova()

set.seed(123)
pairwise.adonis2(aitchison_mat ~ season, data = metadata[1:25,], nperm = 99999)
set.seed(123)
pairwise.adonis2(aitchison_mat ~ location, data = metadata[1:25,], nperm = 99999) # significant difference between Mo i Rana and Røros
```
